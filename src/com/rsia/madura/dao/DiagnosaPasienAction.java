...
* @Date:   2018-05-30 11:05:16
* @Last Modified by:   Pradesga Indonesia
* @Last Modified time: 2018-05-30 11:10:16
*/
...
@Repository
public class DiagnosaPasienAction implements DiagnosaPasienDAO {
	...
	public List<MDiagnosaPasien> getDiagnosaPasiens() {
		...
		Query<MDiagnosaPasien> query = current.createQuery("from MDiagnosaPasien", MDiagnosaPasien.class);

		List<MDiagnosaPasien> diagnosapasien = query.getResultList();
	...
	public List<MDiagnosaPasien> getDiagnosaPasiens(int page, int limit) {
		...
		Query<MDiagnosaPasien> query = current.createQuery("from MDiagnosaPasien", MDiagnosaPasien.class);
		List<MDiagnosaPasien> diagnosapasien = query.getResultList();
		this.total = diagnosapasien.size();
		diagnosapasien = this.getData(page, limit);

		return diagnosapasien;
	}

	public List<MDiagnosaPasien> getData(int page, int limit) {
		Session current = sessionFactory.getCurrentSession();
		Query<MDiagnosaPasien> query = current.createQuery("from MDiagnosaPasien", MDiagnosaPasien.class).setFirstResult((page - 1) * limit)
				.setMaxResults(limit);
		List<MDiagnosaPasien> Result = query.getResultList();
	...
	public MDiagnosaPasien getDiagnosaPasien(int diagnosapasienId) {
		...
		MDiagnosaPasien result = current.get(MDiagnosaPasien.class, diagnosapasienId);
	...
	public int DiagnosaPasienStore(MDiagnosaPasien DiagnosaPasienModel) {
		...
		return (int)current.save(DiagnosaPasienModel);

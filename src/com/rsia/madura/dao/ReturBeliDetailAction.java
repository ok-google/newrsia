...
@Repository
public class ReturBeliDetailAction implements ReturBeliDetailDAO {
	...
	public List<MReturBeliDetail> getReturBeliDetails() {
		...
		Query<MReturBeliDetail> query = current.createQuery("from MReturBeliDetail", MReturBeliDetail.class);
		List<MReturBeliDetail> result = query.getResultList();
	...
	private List<MReturBeliDetail> getData(int page, int limit) {
		Session current = sessionFactory.getCurrentSession();
    	Query<MReturBeliDetail> query = current.createQuery("from MReturBeliDetail", MReturBeliDetail.class).setFirstResult((page-1)*limit).setMaxResults(limit);
        List<MReturBeliDetail> Result = query.getResultList();
	...
	public MReturBeliDetail getReturBeliDetail(int ReturBeliDetailId) {
		...
		MReturBeliDetail result = current.get(MReturBeliDetail.class, ReturBeliDetailId);
	...
	public List<MReturBeliDetail> where(String column, String value) {
		...
		Query<MReturBeliDetail> query = current.createQuery("from MReturBeliDetail where " +
		column + " = " + value, MReturBeliDetail.class);
